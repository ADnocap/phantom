...
We evaluate distributional quality using the Continuous Ranked Probability Score (CRPS), a strictly proper scoring rule defined as
$\text{CRPS}(F, y) = \mathbb{E}_F|X - y| - \tfrac{1}{2}\mathbb{E}_F|X - X'|$
where $X, X'$ are independent draws from the predictive distribution~$F$ and $y$ is the realized value~\cite{gneiting2007}.
