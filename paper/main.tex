...
[TO FILL: JointFM (Hackmann 2026) --- closest related work.
Transformer with Variable-Factored Attention for multivariate
distributional prediction on synthetic SDEs. Achieves oracle
energy distance recovery. Key differences from Phantom:
(1) JointFM predicts absolute process levels, not relative returns;
(2) multivariate joint distributions (N=10 coupled targets) vs
our univariate per-asset; (3) evaluated only on synthetic data,
no real financial data or cross-sectional metrics.
Phantom extends the JointFM paradigm to real cross-sectional
prediction and demonstrates actual OOS signal.
Also mention DeepAR, TFT as probabilistic forecasting baselines.]

\subsection{Probabilistic Forecasting}
[TO FILL: CRPS as proper scoring rule (Gneiting \& Raftery 2007).
Student-t vs Gaussian vs mixture heads. Energy distance.
Why parametric (Student-t) is better than quantile regression for
financial applications --- closed-form density enables NLL training,
tail risk quantification, and position sizing.]
